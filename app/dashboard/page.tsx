...
export default function DashboardPage() {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8">
            ...
            {/* Quick access */}
            <div>
                <h3 className="text-slate-700 font-semibold text-base mb-4">Quick Access</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Link
                        href="/interns"
                        className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md hover:border-blue-200 transition-all group flex items-center gap-4"
                    >
                        ...
                        <div>
                            <p className="font-semibold text-slate-800 text-sm">View All Interns</p>
                            <p className="text-xs text-slate-500 mt-0.5">Filter, search and manage intern records</p>
                        </div>
                    </Link>
                    ...
                    {user?.role === 'admin' && (
                        <Link
                            href="/interns/add"
                            className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md hover:border-green-200 transition-all group flex items-center gap-4"
                        >
                            <div className="w-10 h-10 bg-green-50 rounded-xl flex items-center justify-center group-hover:bg-green-600 transition-colors">
                                <svg className="w-5 h-5 text-green-600 group-hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                                </svg>
                            </div>
                            <div>
                                <p className="font-semibold text-slate-800 text-sm">Add New Intern</p>
                                <p className="text-xs text-slate-500 mt-0.5">Register a new intern to the system</p>
                            </div>
                        </Link>
                    )}
                </div>
